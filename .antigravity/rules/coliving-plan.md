...
> 상세기능은 `functional-specification.md`, 역할/공간/디바이스 정의 참조
...
### 5.2 3대 역할자 + 게스트 (상세는 `functional-specification.md` 참조)
**게스트(🔓):** 방·커뮤니티 글 열람, (선택) VoC 열람
① **USER**: 계약신청(`status` DRAFT/PENDING·연락처 인증), 체결 시 RESIDENT 승격, 커뮤니티·VoC 작성
② **RESIDENT**: 개인/공용 IoT 제어(공용=예약 시간대만), 시설 예약, 계약·기기·예약 이력
③ **ADMIN**: 계약 신청 승인/거절, 직접 등록, 기기 CRUD/제어(전체+CCTV), 공간 CRUD, 배치 D&D, 모니터링, 결제, VoC(`/admin/vocs`)
...
BFF: 브라우저→`/api/...`만 호출, Spring URL은 서버환경변수
