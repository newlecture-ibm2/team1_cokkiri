...
### API 프록시 (app/api/[...path]/route.ts)
클라이언트의 `/api/...` 호출 → Node.js Route Handler가 가로채어 Spring **`INTERNAL_BACKEND_URL/api/...`** 로 포워딩. Session_id 쿠키로 Redis에서 JWT를 추출해 `Authorization: Bearer` 헤더 추가 (`middleware.ts`는 외부 접근 차단용 게이트웨이 역할만 수행).
...
| 37 | `frontend/src/middleware.ts` | 내부 API 외부 접근 차단 게이트웨이 |
