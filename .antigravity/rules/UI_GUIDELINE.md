...
> "우리 프로젝트는 **'Moss & Aloe Editorial'** 가이드를 따라 **Next.js App Router** 기반으로 개발한다. 인터랙션이 있는 컴포넌트는 반드시 상단에 `'use client'`를 명시하고, 모든 색상은 하드코딩하지 말고 `theme.css` 변수를 써라. 타이틀은 `font-black`에 `tracking-tighter`를 적용하고, 페이지 여백은 가이드의 `pt-48`과 `lg:px-24`를 지켜라."
...
### 🔝 네비게이션 및 스크롤 관리 (Navigation & Scroll)
- **Next.js Link 활용**: 모든 페이지 이동은 `next/link`를 사용하여 부드러운 SPA 경험을 유지합니다.
- **자동 스크롤 복구**: Next.js의 내장 기능을 활용하여 페이지 전환 시 자동으로 페이지 최상단부터 노출되도록 관리합니다. 특수한 스크롤 유지가 필요한 경우를 제외하고는 수동 스크롤 컴포넌트 사용을 지양합니다.

---

## 9. DIRECTORY & CO-LOCATION GUIDELINE
Next.js App Router 고유의 완벽한 콜로케이션을 적용하기 위한 핵심 규칙입니다.

**공통 UI 컴포넌트의 경계 설정:** 
- 버튼(Button), 모달(Modal), 레이아웃(Header) 등 팀원 모두가 공통으로 재사용해야 하는 순수 뼈대 컴포넌트는 자신이 맡은 도메인의 `app/(도메인)/_components/` 폴더 안이 아니라, 반드시 도메인 바깥 방어선인 **최상위 전역 `components/`** 폴더에 작성해야 합니다. 
- 복잡한 로직이 포함된 컴포넌트는 가급적 서버 컴포넌트로 유지하되, Framer Motion이나 State가 필요한 경우에만 최소 단위로 `'use client'`를 적용합니다.
