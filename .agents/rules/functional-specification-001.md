...
4.2.5 평면도 배경 설계도 | ADM-SPC-05 | Should — 층별 건물 도면 이미지 업로드/삭제, 투명도 조절. `floor_plans` 테이블의 `blueprint_url`, `blueprint_opacity` 관리. API: `POST/DELETE /api/admin/floors/{floor}/plan/blueprint`, `GET/PUT /api/admin/floors/{floor}/plan`
4.2.6 평면도 비공간 요소(어노테이션) | ADM-SPC-06 | Should — DOOR/STAIRS/GARDEN/ELEVATOR/RESTROOM/CUSTOM 아이콘 배치. `floor_plans.annotations` JSONB 저장. D&D 배치 + 더블클릭 라벨 편집
...
Next Route Handler(`app/api/[...path]/route.ts`)가 프론트엔드의 **`/api/...`** 요청을 가로채어 Spring **`/api/...`** 와 **동일 경로**로 프록시한다(예: 클라이언트 `GET /api/rooms` → Route Handler → Spring `GET /api/rooms`). 과거의 `/bff` 세그먼트는 더 이상 사용하지 않는다. URL에 `/user/`, `/resident/` 등 역할 프리픽스를 두는 방식은 **필수 아님** — 인가는 Spring Security·`@PreAuthorize`로 처리. Mock IoT 등 예외 경로만 별도 라우팅.
...
**게스트(🔓)**: 방 목록/상세, 공용시설 소개(문/상세), **층별 평면도 열람**, 커뮤니티 **게시글 목록·상세 열람**, (정책 선택) VoC 목록·상세 열람
...
| 공용시설 소개 | USR-EXP-01 | 공용시설 목록/상세, 이미지 갤러리 | Should |
| 층별 평면도 | USR-FLR-01 | 인터랙티브 평면도 뷰어, 클릭→상세 이동 | Should |
