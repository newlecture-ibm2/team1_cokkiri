...
## 3-1. Experience (공용시설 소개)
3-1.1 공용시설목록 | GET `/experience` | 🔓 | 공용시설 소개 페이지용 (예약 가능 여부 무관 전체 조회)
3-1.2 공용시설상세 | GET `/experience/{spaceId}` | 🔓 | 단일 공용시설 상세 조회
...
11.5 평면도조회 | GET `/admin/floors/{floor}/plan` | 🔒 | blueprint + annotations 조회
11.6 평면도저장 | PUT `/admin/floors/{floor}/plan` | 🔒 | Req:{blueprintOpacity, annotations}
11.7 배경도면업로드 | POST `/admin/floors/{floor}/plan/blueprint` | 🔒 | Req:multipart(file)
11.8 배경도면삭제 | DELETE `/admin/floors/{floor}/plan/blueprint` | 🔒
...
12.2 등록/수정/삭제 | POST, PUT, DELETE `/admin/devices[/id]` | 🔒 | Req:{spaceId,device_type_id,name,modelName,macAddress,mockEndpoint}
...
14.5 커뮤니티관리(게시글/댓글) | GET `/admin/posts`, GET `/admin/posts/{id}`, DELETE `/admin/posts/{id}`, GET `/admin/comments`, DELETE `/admin/comments/{id}` | 🔒 | Query:category,status,author,p,s,sort (관리자 검수·블라인드/삭제 목적)

## 15. Admin (User)
15.1 사용자목록조회 | GET `/admin/users` | 🔒 | Query:role,status,name,loginId,p,s
15.2 사용자상세조회 | GET `/admin/users/{id}` | 🔒
15.3 비활성화(정지) | PATCH `/admin/users/{id}/deactivate` | 🔒
15.4 권한변경 | PATCH `/admin/users/{id}/role` | 🔒 | Req:{role(USER|ADMIN)}
15.5 개인정보수정 | PUT `/admin/users/{id}` | 🔒 | Req:{name,phone,email}

## 16. Floor Map (Public)
16.1 층별평면도조회 | GET `/floors` | 🔓 | 층 목록 + 공간 블록 + 어노테이션 + 배경 이미지 (읽기 전용, 민감 정보 제외)
