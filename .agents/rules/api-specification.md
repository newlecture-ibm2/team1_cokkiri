...
## 9. VoC
9.1 등록/내목록/상세/수정/취소 | POST `/voc`, GET `/voc/my`, GET `/voc/{id}`, PUT, POST `/voc/{id}[/cancel]` | 🔑 | POST Req:multipart(category,title,content,files)
...
14.4 민원목록/상세/답변/처리완료 | GET `/admin/voc`, GET `/admin/voc/{id}`, POST `/admin/voc/{id}/reply`, POST `/admin/voc/{id}/resolve` | 🔒 | Query:status,p,s
ㄴ