...
### 1.6 토큰갱신 | CMN-AUTH-03 | Must — RefreshToken→새토큰발급, RESIDENT: 계약상태재확인(만료→role=USER)
...
### 1.10 커뮤니티 | 🔑Auth(USER이상)
1.10.1 게시글목록/상세 | CMN-CMT-01 | Should — 표시: 유형(공지/질문/제안/모임/자유),프사,제목,작성자,작성일,댓글수,좋아요,조회수. 최신순+페이지네이션
...
입력: spaceId(AVAILABLE PRIVATE만), name, birthDate, gender, phone(**SMS인증필수**), nationality, address, email(**이메일인증필수**), bankAccount, desiredStartDate(오늘이후), desiredDuration(1개월+), contractLanguage(한국어/영어), privacyConsent, requestNotes(500자)
버튼: 임시저장(→DRAFT, 재진입시자동복원) / 신청(→인증완료확인→PENDING)
검증: 호실AVAILABLE+진행중신청없음+인증완료→CONTRACT INSERT(PENDING)→관리자알림
승인후 유저가 계약체결화면에서 최종동의 필요
...
다음 로그인 시 JWT에 contract_id, space_id 포함
...
### 2.7 활동이력 | USR-HST-01 | Should — 탭: 신청이력/계약이력/게시글이력/댓글이력. 각 최신순정렬
...
`/api/bff/auth/**`→AuthController, `/api/bff/public/**`→PublicAPI(인증불필요), `/api/bff/user/**`→UserAPI(USER+), `/api/bff/admin/**`→AdminAPI(ADMIN), `/api/bff/resident/**`→ResidentAPI(RESIDENT), `/api/bff/devices/*/control`→DeviceControl→MockIoT
...
**Admin전용(🔒)**: 대시보드, 공간CRUD, 기기CRUD+제어(전체), 계약CRUD+승인거절, 예약승인, 결제, 모니터링, 민원관리

**공통(🔑)**: 내정보조회/수정, PW변경, 탈퇴, 활동이력, 커뮤니티(게시글CRUD+댓글, ADMIN=전체삭제권), 민원등록/조회

**USER+(👤)**: 방조회(🔓Public), 계약신청/체결/조회

**RESIDENT+(🏠)**: IoT기기제어(space_id기반, 공용=예약시간대만), 시설예약(신청/조회/취소), 기기사용기록, 예약이력

- ADMIN: 게시글/댓글/문의 전체수정삭제(관리목적), IoT전체공간제어
- RESIDENT/USER: 본인게시글/댓글/문의만 수정삭제
