...
export function Header() {
  ...
  return (
    <motion.header
      className={`sticky top-0 z-[100] border-b px-6 py-2 transition-all duration-500 md:px-12 md:py-3 ${isScrolled
          ...
          <nav className="hidden items-center gap-10 md:flex">
            {navLinks.map((link) => (
              <Link
                key={link.path}
                to={link.path}
                className={`text-sm font-black tracking-widest text-[#2C3424] uppercase transition-all duration-500 hover:opacity-100 ${location.pathname === link.path
