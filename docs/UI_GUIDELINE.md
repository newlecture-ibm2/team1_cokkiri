...
## 8. DIRECTORY & CO-LOCATION GUIDELINE
**주의 및 고려사항**
App Router 고유의 완벽한 콜로케이션을 적용하실 때 딱 한 가지만 조심하시면 됩니다.

**공통 UI 컴포넌트의 경계 설정:** 버튼(Button), 모달(Modal), 레이아웃(Header) 등 팀원 모두가 공통으로 재사용해야 하는 순수 뼈대 컴포넌트는 자신이 맡은 도메인의 `app/(도메인)/_components/` 폴더 안이 아니라, 반드시 도메인 바깥 방어선인 **최상위 전역 `components/`** 폴더에 작성해야 합니다. 즉, "이 컴포넌트가 내 경로(Route) 안에서만 독점적으로 쓰이는가?"를 기준으로 폴더 위치 결정에 신중해야 합니다.
