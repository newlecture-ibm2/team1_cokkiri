...
-- DROP TABLE IF EXISTS 순서: FK 참조의 역순으로 삭제
...
CREATE TABLE users (
    ...
    phone           VARCHAR(20)     NOT NULL,                   -- 000-0000-0000 형식
    email           VARCHAR(255)    NOT NULL,
    role            VARCHAR(20)     NOT NULL DEFAULT 'USER'
                        CHECK (role IN ('USER', 'RESIDENT', 'ADMIN')),
    status          VARCHAR(20)     NOT NULL DEFAULT 'ACTIVE'
                        CHECK (status IN ('ACTIVE', 'DEACTIVATED')),
    profile_image   VARCHAR(500),                               -- 프로필 사진 경로
...
CREATE TABLE space (
    ...
    floor               INTEGER,
    area                NUMERIC(10, 2),                         -- 면적 (㎡)

    -- PRIVATE 전용 필드
    room_type           VARCHAR(20)
                            CHECK (room_type IN ('SINGLE', 'DOUBLE', 'STUDIO', 'SUITE') OR room_type IS NULL),
    room_count          INTEGER,                                -- 방 수
    bathroom_count      INTEGER,                                -- 욕실 수
    direction           VARCHAR(10),                            -- 남향/북향/동향/서향
    deposit             NUMERIC(15, 0),                         -- 보증금 (원)
    monthly_rent        NUMERIC(15, 0),                         -- 월 임대료 (원)
    maintenance_fee     NUMERIC(15, 0),                         -- 관리비 (원)
    parking_available   BOOLEAN         DEFAULT FALSE,          -- 주차 가능 여부

    -- COMMON 전용 필드
    max_capacity        INTEGER,                                -- 최대 수용 인원
    operating_hours     VARCHAR(20),                            -- 운영 시간 (예: 06:00-23:00)
    is_reservable       BOOLEAN         DEFAULT FALSE,          -- 예약 가능 여부

    -- 공통 필드
    amenities           TEXT,                                   -- 생활 시설 (JSON)
    description         TEXT,                                   -- 상세 설명
    position_x          INTEGER,                                -- 배치 좌표 X (대시보드용)
    position_y          INTEGER,                                -- 배치 좌표 Y (대시보드용)
    created_at          TIMESTAMPTZ     NOT NULL DEFAULT NOW(),
    updated_at          TIMESTAMPTZ     NOT NULL DEFAULT NOW()
);
...
COMMENT ON COLUMN space_image.is_thumbnail IS '대표 이미지 여부';
...
CREATE TABLE device_type (
    id                  BIGSERIAL       PRIMARY KEY,
    code                VARCHAR(30)     NOT NULL UNIQUE,         -- 예: LIGHT, HVAC, LOCK
    name                VARCHAR(50)     NOT NULL,                -- 예: 스마트조명
    commands            JSONB           NOT NULL DEFAULT '[]',   -- 지원 명령 목록
    ui_type             VARCHAR(30),                             -- toggle / slider / button 등
...
COMMENT ON COLUMN device_type.code IS '코드: LIGHT, HVAC, LOCK, WASHER, DRYER, CCTV, HEATER';
COMMENT ON COLUMN device_type.commands IS '지원 명령 목록 (JSON 배열)';
COMMENT ON COLUMN device_type.is_system_default IS '시스템 기본 제공 기기 종류 여부';
...
CREATE TABLE device (
    ...
    name            VARCHAR(100)    NOT NULL,                    -- 관리자 지정 기기명
    model_name      VARCHAR(100),                                -- 제조사 모델명
    mock_endpoint   VARCHAR(500),                                -- 목업 IoT 엔드포인트 URL
...
COMMENT ON COLUMN device.is_active IS 'true: 활성, false: 비활성 (입주자에게 미표시)';
...
CREATE TABLE booking (
    ...
    -- 신청자 정보 (회원정보와 별도로 저장하여 신청 시점 정보 보존)
    applicant_name          VARCHAR(50)     NOT NULL,
    birth_date              VARCHAR(6)      NOT NULL,
    gender                  VARCHAR(10)     NOT NULL
                                CHECK (gender IN ('MALE', 'FEMALE')),
    nationality             VARCHAR(100)    NOT NULL,
    phone                   VARCHAR(20)     NOT NULL,           -- 인증 완료된 연락처
    address                 VARCHAR(500)    NOT NULL,
    email                   VARCHAR(255)    NOT NULL,           -- 인증 완료된 이메일
    bank_account            VARCHAR(100)    NOT NULL,           -- 은행명 + 계좌번호

    -- 신청 조건
    desired_start_date      DATE            NOT NULL,           -- 희망 입주일
    desired_duration_months INTEGER         NOT NULL,           -- 희망 계약 기간 (월)
    contract_language       VARCHAR(5)      NOT NULL DEFAULT 'KO'
                                CHECK (contract_language IN ('KO', 'EN')),
    privacy_agreed          BOOLEAN         NOT NULL DEFAULT FALSE,
    request_note            VARCHAR(500),                       -- 요청 사항 (최대 500자)

    -- 관리자 승인 시 확정 조건
    confirmed_rent          NUMERIC(15, 0),                     -- 확정 월 임대료
    confirmed_deposit       NUMERIC(15, 0),                     -- 확정 보증금
    confirmed_start_date    DATE,                               -- 확정 계약 시작일
    confirmed_end_date      DATE,                               -- 확정 계약 종료일
...
COMMENT ON COLUMN booking.status IS '상태: DRAFT(임시저장) / PENDING(신청) / APPROVED(승인) / REJECTED(거절) / CANCELLED(취소) / CONTRACTED(계약완료)';
...
CREATE TABLE contract (
    ...
    user_id         BIGINT          NOT NULL REFERENCES users(id),
    space_id        BIGINT          NOT NULL REFERENCES space(id),
    booking_id      BIGINT          REFERENCES booking(id),     -- 유저 주도 시 연결, 관리자 주도 시 NULL
    status          VARCHAR(15)     NOT NULL DEFAULT 'ACTIVE'
                        CHECK (status IN ('ACTIVE', 'EXPIRED', 'TERMINATED')),
    start_date      DATE            NOT NULL,
    end_date        DATE            NOT NULL,
    monthly_rent    NUMERIC(15, 0)  NOT NULL,                   -- 월 임대료 (원)
    deposit         NUMERIC(15, 0),                              -- 보증금 (원)
    special_terms   TEXT,                                        -- 특약 사항
    contracted_at   TIMESTAMPTZ     NOT NULL DEFAULT NOW(),     -- 계약 체결일
    created_at      TIMESTAMPTZ     NOT NULL DEFAULT NOW(),
    updated_at      TIMESTAMPTZ     NOT NULL DEFAULT NOW(),

    -- 비즈니스 규칙: 한 시점에 하나의 PRIVATE 호실에는 ACTIVE 계약이 최대 1개
    CONSTRAINT uq_contract_space_active UNIQUE (space_id, status)
        -- 참고: 이 UNIQUE 제약은 ACTIVE일 때만 적용하려면 아래 partial unique index 사용
);

-- partial unique index: ACTIVE 상태 계약에 대해서만 space_id 유니크
DROP INDEX IF EXISTS uq_active_contract_per_space;
...
-- uq_contract_space_active 는 partial index와 중복이므로 제거
ALTER TABLE contract DROP CONSTRAINT IF EXISTS uq_contract_space_active;
...
COMMENT ON COLUMN reservation.status IS '상태: PENDING(대기) / APPROVED(승인) / CANCELLED(취소) / COMPLETED(완료)';
...
CREATE TABLE control_log (
    ...
    command         VARCHAR(50)     NOT NULL,                   -- TURN_ON, SET_TEMP 등
    command_params  JSONB,                                      -- 명령 파라미터 (예: {"temp": 24})
    result          VARCHAR(10)     NOT NULL
                        CHECK (result IN ('SUCCESS', 'FAILURE')),
    error_message   TEXT,                                       -- 실패 시 에러 메시지
    correlation_id  VARCHAR(100),                               -- 요청 추적 ID
...
COMMENT ON COLUMN control_log.actor_type IS '수행자 유형: RESIDENT(입주자) / ADMIN(관리자)';
...
CREATE TABLE payment (
    ...
    amount          NUMERIC(15, 0)  NOT NULL,                   -- 결제 금액 (원)
...
CREATE TABLE post_attachment (
    ...
    file_size       BIGINT          NOT NULL,                   -- 파일 크기 (bytes), 최대 10MB
    created_at      TIMESTAMPTZ     NOT NULL DEFAULT NOW()
);
...
CREATE TABLE comment (
    ...
    post_id         BIGINT          NOT NULL REFERENCES post(id) ON DELETE CASCADE,
    author_id       BIGINT          NOT NULL REFERENCES users(id),
    content         TEXT            NOT NULL,                    -- 줄바꿈 보존 필요 (\n → <br>)
    created_at      TIMESTAMPTZ     NOT NULL DEFAULT NOW(),
    updated_at      TIMESTAMPTZ     NOT NULL DEFAULT NOW()
);
...
CREATE TABLE voc (
    ...
    replied_by      BIGINT          REFERENCES users(id),       -- 답변 관리자 ID
...
COMMENT ON COLUMN voc.status IS '상태: OPEN(접수) / IN_PROGRESS(처리중) / RESOLVED(완료) / CANCELLED(취소)';
...
CREATE TABLE notification (
    id              BIGSERIAL       PRIMARY KEY,
    user_id         BIGINT          NOT NULL REFERENCES users(id),
    type            VARCHAR(30)     NOT NULL,                   -- BOOKING_APPROVED, CONTRACT_CREATED 등
    title           VARCHAR(200)    NOT NULL,
    message         TEXT            NOT NULL,
    reference_type  VARCHAR(20),                                -- BOOKING / CONTRACT / RESERVATION / VOC
    reference_id    BIGINT,                                     -- 참조 엔터티 ID
...
COMMENT ON COLUMN notification.reference_type IS '참조 엔터티 유형: BOOKING / CONTRACT / RESERVATION / VOC';
...
CREATE TABLE role_change_log (
    ...
    reason          VARCHAR(255),                               -- CONTRACT_CREATED, CONTRACT_EXPIRED 등
    contract_id     BIGINT          REFERENCES contract(id),
    changed_by      BIGINT,                                     -- 변경 수행자 ID (관리자 또는 SYSTEM=0)
...
CREATE TABLE token_blacklist (
    id              BIGSERIAL       PRIMARY KEY,
    token_jti       VARCHAR(255)    NOT NULL UNIQUE,            -- JWT의 JTI 클레임
    expires_at      TIMESTAMPTZ     NOT NULL,                   -- 원래 토큰 만료 시각
    reason          VARCHAR(255),                               -- LOGOUT, CONTRACT_TERMINATED 등
    created_at      TIMESTAMPTZ     NOT NULL DEFAULT NOW()
);
...
-- ──────────────────────────────────────────────
-- 관리자 계정 기본값 (비밀번호: Admin1234!)
-- ──────────────────────────────────────────────
-- 참고: password_hash는 BCrypt 해시값으로, 실제 배포 시 변경해야 합니다.

INSERT INTO users (login_id, password_hash, name, birth_date, gender, nationality, phone, email, role, status)
VALUES (
    'admin',
    '$2a$10$dummyhashvaluefordevpurposesonlynotreal',  -- 실제 BCrypt 해시로 교체 필요
    '관리자',
    '900101',
    'MALE',
    '대한민국',
...
CREATE TRIGGER trg_space_updated_at
    BEFORE UPDATE ON space FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER trg_device_type_updated_at
    BEFORE UPDATE ON device_type FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER trg_booking_updated_at
    BEFORE UPDATE ON booking FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER trg_contract_updated_at
    BEFORE UPDATE ON contract FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER trg_reservation_updated_at
    BEFORE UPDATE ON reservation FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER trg_payment_updated_at
    BEFORE UPDATE ON payment FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER trg_post_updated_at
    BEFORE UPDATE ON post FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER trg_comment_updated_at
    BEFORE UPDATE ON comment FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER trg_voc_updated_at
    BEFORE UPDATE ON voc FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
