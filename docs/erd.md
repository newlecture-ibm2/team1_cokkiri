...
> [!NOTE]
> 기능 명세서(functional_specification.md) 기반으로 도출한 데이터베이스 설계입니다.
> MVP 범위의 핵심 엔터티와 관계를 정의합니다.
...
erDiagram
    ...
    USER {
        ...
        VARCHAR birth_date "생년월일 (YYMMDD)"
        ENUM gender "성별 (MALE / FEMALE)"
        VARCHAR nationality "국적"
        VARCHAR phone "연락처 (000-0000-0000)"
        VARCHAR email "이메일"
        ENUM role "역할 (USER / RESIDENT / ADMIN)"
        ENUM status "계정 상태 (ACTIVE / DEACTIVATED)"
    ...
    %% ──────────────────────────────────────────────
    SPACE {
        BIGINT id PK "공간 고유 ID"
        VARCHAR name UK "공간명 (1~100자, 중복 불가)"
        ENUM type "유형 (PRIVATE / COMMON)"
        ENUM status "상태 (AVAILABLE / OCCUPIED / MAINTENANCE)"
        INT floor "층"
        DECIMAL area "면적 (㎡)"
        INT max_capacity "최대 수용 인원 (COMMON 전용)"
        VARCHAR operating_hours "운영 시간 (COMMON 전용, 예: 06:00-23:00)"
        BOOLEAN is_reservable "예약 가능 여부 (COMMON만 true 가능)"
        INT room_count "방 수"
        INT bathroom_count "욕실 수"
        VARCHAR direction "방향 (남향/북향/동향/서향)"
        DECIMAL deposit "보증금 (원)"
        DECIMAL monthly_rent "월 임대료 (원)"
        DECIMAL maintenance_fee "관리비 (원)"
        BOOLEAN parking_available "주차 가능 여부"
        TEXT amenities "생활 시설 (JSON 또는 CSV)"
        TEXT description "상세 설명"
        VARCHAR room_type "방 유형 (1인실/2인실 등)"
        INT position_x "배치 좌표 X (대시보드용)"
        INT position_y "배치 좌표 Y (대시보드용)"
    ...
    %% ──────────────────────────────────────────────
    SPACE_IMAGE {
        BIGINT id PK "이미지 고유 ID"
        BIGINT space_id FK "공간 ID"
        VARCHAR image_url "이미지 URL"
        ENUM image_type "유형 (PHOTO / FLOOR_PLAN)"
    ...
    %% ──────────────────────────────────────────────
    DEVICE_TYPE {
        BIGINT id PK "기기 종류 고유 ID"
        VARCHAR code UK "코드 (예: LIGHT, HVAC, LOCK, WASHER, DRYER, CCTV)"
        VARCHAR name "표시명 (예: 스마트조명)"
        TEXT commands "지원 명령 목록 (JSON)"
    ...
    DEVICE {
        ...
        ENUM status "연결 상태 (ONLINE / OFFLINE / ERROR)"
        BOOLEAN is_active "활성화 여부"
        TIMESTAMP installed_at "설치일"
        TIMESTAMP last_online_at "마지막 온라인 시각"
        TIMESTAMP created_at "생성일"
        TIMESTAMP updated_at "수정일"
    }
    ...
    %% ──────────────────────────────────────────────
    BOOKING {
        BIGINT id PK "신청 고유 ID"
        BIGINT user_id FK "신청자 ID"
        BIGINT space_id FK "희망 호실 ID"
        ENUM status "상태 (DRAFT / PENDING / APPROVED / REJECTED / CANCELLED / CONTRACTED)"
        ...
        INT desired_duration_months "희망 계약 기간 (월)"
        ENUM contract_language "계약서 언어 (KO / EN)"
        BOOLEAN privacy_agreed "개인정보 동의"
        TEXT request_note "요청 사항 (최대 500자)"
        DECIMAL confirmed_rent "확정 월 임대료 (승인 시)"
        DECIMAL confirmed_deposit "확정 보증금 (승인 시)"
        DATE confirmed_start_date "확정 계약 시작일 (승인 시)"
        DATE confirmed_end_date "확정 계약 종료일 (승인 시)"
        TEXT special_terms "특약 사항 (승인 시)"
        TIMESTAMP created_at "신청일"
        TIMESTAMP updated_at "수정일"
    }
    ...
    RESERVATION {
        ...
        ENUM status "상태 (PENDING / APPROVED / CANCELLED / COMPLETED)"
    ...
    CONTROL_LOG {
        ...
        ENUM actor_type "수행자 유형 (RESIDENT / ADMIN)"
        VARCHAR command "제어 명령 (TURN_ON, SET_TEMP 등)"
        TEXT command_params "명령 파라미터 (JSON, 예: {temp: 24})"
        ENUM result "결과 (SUCCESS / FAILURE)"
        TEXT error_message "실패 시 에러 메시지"
        VARCHAR correlation_id "요청 추적 ID"
        TIMESTAMP executed_at "실행 시각"
    }
    ...
    PAYMENT {
        ...
        ENUM type "유형 (RENT / MAINTENANCE / FACILITY)"
        DECIMAL amount "결제 금액 (원)"
        ENUM status "상태 (UNPAID / PENDING / PAID)"
    ...
    POST {
        ...
        ENUM category "유형 (NOTICE / QUESTION / SUGGESTION / MEETUP / FREE)"
    ...
    %% ──────────────────────────────────────────────
    VOC {
        BIGINT id PK "민원 고유 ID"
        BIGINT user_id FK "문의자 ID"
        VARCHAR title "제목"
        TEXT content "문의 내용"
        ENUM status "상태 (OPEN / IN_PROGRESS / RESOLVED / CANCELLED)"
    ...
    %% ──────────────────────────────────────────────
    TOKEN_BLACKLIST {
        BIGINT id PK "블랙리스트 고유 ID"
        VARCHAR token_jti UK "토큰 JTI (고유 식별자)"
        TIMESTAMP expires_at "원래 만료 시각"
    ...
    USER ||--o{ BOOKING : "신청한다"
    USER ||--o{ CONTRACT : "계약한다"
    USER ||--o{ RESERVATION : "예약한다"
    USER ||--o{ POST : "작성한다"
    USER ||--o{ COMMENT : "댓글단다"
    USER ||--o{ POST_LIKE : "좋아요한다"
    USER ||--o{ VOC : "문의한다"
    USER ||--o{ CONTROL_LOG : "기기를 제어한다"
    USER ||--o{ PAYMENT : "결제한다"
    USER ||--o{ ROLE_CHANGE_LOG : "역할이 변경된다"
    USER ||--o{ REFRESH_TOKEN : "토큰을 보유한다"
...
| # | 엔터티 | 설명 | 관련 기능 ID |
|---|---|---|---|
| 1 | **USER** | 사용자 (USER / RESIDENT / ADMIN) | CMN-AUTH-00~03, CMN-PRF-01~04 |
| 2 | **SPACE** | 공간 (개인 호실 / 공용 시설) | USR-ROM-01~02, ADM-SPC-01~03 |
...
| 6 | **BOOKING** | 계약 신청 (임시저장/신청/승인/거절) | USR-CTR-00~00-1, ADM-BKG-01 |
| 7 | **CONTRACT** | 임대차 계약 | USR-CTR-01~02, RES-CTR-01~02, ADM-CTR-01~04 |
| 8 | **RESERVATION** | 공용 시설 예약 | RES-RSV-01~04, ADM-RSV-01~02 |
| 9 | **CONTROL_LOG** | IoT 기기 제어 이력 | RES-DEV-02, RES-LOG-01, ADM-DEV-04, ADM-MON-02 |
| 10 | **PAYMENT** | 결제 / 정산 | ADM-BIL-01, CMN-PRF-04 |
| 11 | **POST** | 커뮤니티 게시글 | CMN-CMT-01~02 |
| 12 | **POST_ATTACHMENT** | 게시글 첨부파일 | CMN-CMT-02 |
| 13 | **POST_LINK** | 게시글 URL 링크 | CMN-CMT-02 |
| 14 | **POST_LIKE** | 게시글 좋아요 | CMN-CMT-01 |
| 15 | **COMMENT** | 댓글 | CMN-CMT-03 |
| 16 | **VOC** | 민원 / 문의 | ADM-VOC-01 |
| 17 | **ROLE_CHANGE_LOG** | 역할 변경 이력 | USR-CTR-01, ADM-CTR-02~04 |
| 18 | **REFRESH_TOKEN** | JWT 리프레시 토큰 관리 | CMN-AUTH-01~04 |
| 19 | **TOKEN_BLACKLIST** | 무효화된 토큰 관리 | CMN-AUTH-02, ADM-CTR-04 |

---

## 3. 주요 관계 설명

### 3.1 USER ↔ SPACE (다대다, CONTRACT를 통해)

```
USER ─(1:N)─ CONTRACT ─(N:1)─ SPACE
```
...
- **개인 기기**: JWT의 `space_id`와 DEVICE의 `space_id`가 일치해야 제어 가능
- **공용 기기**: 현재 시각 기준 해당 시설에 APPROVED 예약이 있어야 제어 가능
- **관리자**: `space_id` 제한 없이 전체 기기 제어 가능
...
| ENUM 이름 | 값 | 사용처 |
|---|---|---|
| **user_role** | `USER`, `RESIDENT`, `ADMIN` | USER.role |
| **user_status** | `ACTIVE`, `DEACTIVATED` | USER.status |
| **gender** | `MALE`, `FEMALE` | USER.gender, BOOKING.gender |
...
| 2 | `RESIDENT`로 승격된 유저는 반드시 `ACTIVE` 상태의 `CONTRACT`를 보유해야 한다 | USER, CONTRACT |
| 3 | 계약 만료/해지 시 `SPACE.status`를 `AVAILABLE`로, `USER.role`을 `USER`로 복원해야 한다 | SPACE, USER, CONTRACT |
...
| 7 | `NOTICE` 유형의 게시글은 `ADMIN` 역할만 작성 가능하다 | POST, USER |
| 8 | 활성 계약 또는 미납금이 있는 사용자는 회원 탈퇴 불가 | USER, CONTRACT, PAYMENT |
...
> [!TIP]
> 이 ERD는 기능 명세서의 **MVP 범위**를 기준으로 설계되었습니다.
> 향후 확장 시 공간 종류, 디바이스 종류, 에너지 계량 등의 테이블이 추가될 수 있습니다.
