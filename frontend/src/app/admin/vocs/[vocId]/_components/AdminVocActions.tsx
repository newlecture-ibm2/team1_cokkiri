...
export function AdminVocActions({ vocId, status }: Props) {
  ...
  return (
    <div className="mt-10 space-y-8 border-t border-border pt-10">
      ...
      {canResolve && (
        <div className="flex flex-wrap items-center justify-between gap-4 rounded-[2rem] border border-[#7F1D1D]/20 bg-[#7F1D1D]/5 p-6">
          <p className="max-w-md text-sm font-medium tracking-tight text-[#7F1D1D]">
          ...
          <motion.button
            ...
            onClick={resolveVoc}
            className={cn(
              "inline-flex items-center gap-2 rounded-xl border border-[#7F1D1D] bg-[#7F1D1D]/10 px-6 py-3 text-sm font-black uppercase tracking-wider text-[#7F1D1D]",
              pending && "opacity-60",
            )}
          >
